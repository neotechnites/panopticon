...
func _frame_the_map() -> void:
	...
	if route != null and route.level_count() > 0:
		...
		if top != null:
			radius = maxf(top.outer_radius, radius)
			centre.y += route.deck_height(route.last_index()) * 0.5
	...
	# Already unit length: sin^2 + cos^2 is 1, so the multiplier below is metres.
