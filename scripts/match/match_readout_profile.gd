...
@export var guard_role: String = "HUD_ROLE_TOWER"

## The role line for a living prisoner on the ring.
@export var prisoner_role: String = "HUD_ROLE_PRISONER"

## The role line for a ghost.
@export var ghost_role: String = "HUD_ROLE_GHOST"
...
@export var spectator_role: String = "HUD_ROLE_WATCHING"
...
@export var distance_suffix: String = "HUD_DISTANCE_SUFFIX"

## Shown instead of a distance once this participant's lap has been scored.
@export var arrived_text: String = "HUD_ARRIVED"
...
@export var ghost_orders: String = "HUD_GHOST_ORDERS"
...
@export var ghost_take_place_text: String = "HUD_GHOST_TAKE_PLACE"

## Follows the ghost's speed multiplier, e.g. "3.0x FASTER".
@export var ghost_speed_suffix: String = "HUD_GHOST_SPEED_SUFFIX"

## Shown for a ghost while [member GhostProfile.shootable] is false.
@export var ghost_unshootable_text: String = "HUD_GHOST_UNSHOOTABLE"
...
@export var prisoners_word: String = "HUD_PRISONERS"

# --- The rules in play, said out loud -----------------------------------------

## [constant MatchRules.ShooterWinCondition.TOTAL_CONVERSION].
@export var total_conversion_objective: String = "HUD_OBJECTIVE_TOTAL_CONVERSION"
...
@export var shutout_objective: String = "HUD_OBJECTIVE_SHUTOUT"
...
@export var hold_duration_objective: String = "HUD_OBJECTIVE_HOLD"
...
@export var unwinnable_objective: String = "HUD_OBJECTIVE_UNWINNABLE"

## [constant MatchRules.RunnerWinCondition.FIRST_ARRIVAL].
@export var first_arrival_objective: String = "HUD_OBJECTIVE_FIRST_ARRIVAL"

## [constant MatchRules.RunnerWinCondition.ALL_ARRIVALS].
@export var all_arrivals_objective: String = "HUD_OBJECTIVE_ALL_ARRIVALS"

## The opening race, which has no shooter and no win condition yet.
@export var race_objective: String = "HUD_OBJECTIVE_RACE"
...
@export var final_round_text: String = "HUD_FINAL_ROUND"

## Prefixes "n / m" round wins when the match takes more than one.
@export var round_wins_word: String = "HUD_ROUND_WINS"

# --- Context ------------------------------------------------------------------

## Shown where the seat holder's name would go during the opening race.
@export var empty_tower_text: String = "HUD_TOWER_EMPTY"
...
@export var reload_length_suffix: String = "HUD_RELOAD_LENGTH_SUFFIX"
...
@export var restart_hint: String = "HUD_RESTART_HINT"
...
func shooter_objective(rules: MatchRules) -> String:
	...
	if not rules.is_shooter_win_condition_implemented():
		return tr(unwinnable_objective)
	match rules.shooter_win_condition:
		MatchRules.ShooterWinCondition.SHUTOUT_COUNT:
			if rules.shutout_count <= 0:
				return tr(unwinnable_objective)
			return tr(shutout_objective).format({"count": rules.shutout_count})
		MatchRules.ShooterWinCondition.HOLD_DURATION:
			if rules.hold_duration_seconds <= 0.0:
				return tr(unwinnable_objective)
			return tr(hold_duration_objective).format({"seconds": int(roundf(rules.hold_duration_seconds))})
	return tr(total_conversion_objective)
...
func runner_objective(rules: MatchRules) -> String:
	...
	if rules.runner_win_condition == MatchRules.RunnerWinCondition.ALL_ARRIVALS:
		return tr(all_arrivals_objective)
	return tr(first_arrival_objective)
...
func match_progress(rules: MatchRules, rounds_won: int) -> String:
	...
	if needed <= 1:
		return tr(final_round_text)
	return "%s %d / %d" % [tr(round_wins_word), maxi(rounds_won, 0), needed]


## "PRISONERS n / m", the one count both roles read.
func prisoner_count_text(remaining: int, total: int) -> String:
	return "%s %d / %d" % [tr(prisoners_word), maxi(remaining, 0), maxi(total, 0)]
