#!/usr/bin/env bash
# Render the deliverable from cuts\timeline.mp4 on the PC: aspect, captions, voice.
#
...
# voice\voice_NN.wav is laid at shot NN's gap. Output: final\final.mp4, or
# final\final__speak-at-<t>s-for-<g>s.mp4 while a gap still has no recording, so
# the filename says when to talk. --still writes frames\final_still.png and
# pulls it to the Mac.
set -euo pipefail
...
BEATS=$(mktemp)
pc_pull "$(ff "${DIR}")/cuts/beats.txt" "${BEATS}"
FILTER="[0:v]${GEOMETRY}"
CAPTIONS=$(mktemp -d)
GAP_MARK=""
DIR_FF=$(ff "${DIR}" | sed 's/:/\\:/')
while IFS=$'\t' read -r n start end gap_start gap_end caption said; do
  ...
  if [ -n "${caption}" ] && [ "${caption}" != "-" ]; then
    ...
    pc_push "${CAPTIONS}/${NN}.txt" "$(ff "${DIR}")/notes/caption_${NN}.txt"
    FILTER="${FILTER},drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':textfile='${DIR_FF}/notes/caption_${NN}.txt':fontcolor=white:shadowcolor=black@0.7:shadowx=2:shadowy=2:fontsize=${CAPTION_SIZE}:x=(w-text_w)/2:y=${CAPTION_Y}:enable='between(t,${start},${end})'"
  fi
...
\$mix = ''
\$k = 0
foreach (\$w in (Get-ChildItem "\$dir\\voice\\voice_*.wav" -ErrorAction SilentlyContinue | Sort-Object Name)) {
  \$nn = \$w.BaseName -replace 'voice_', ''
  \$beat = Get-Content "\$dir\\cuts\\beats.txt" | Where-Object { \$_ -match "^\$([int]\$nn)\`t" } | Select-Object -First 1
...
if (\$k -gt 0) {
  ...
  \$out = "\$dir\\final\\final.mp4"
} else {
  \$afilter = "[0:a]loudnorm=I=-16:TP=-1.5:LRA=11[aout]"
  \$out = "\$dir\\final\\final${GAP_MARK}.mp4"
}
Remove-Item "\$dir\\final\\final*.mp4" -ErrorAction SilentlyContinue
ffmpeg -hide_banner -loglevel error -y -i "\$dir\\cuts\\timeline.mp4" @(\$inputs) -filter_complex "${FILTER};\$afilter" -map '[vout]' -map '[aout]' -c:v libx264 -preset medium -crf 19 -pix_fmt yuv420p -r ${FPS} -c:a aac -ar 48000 -b:a 192k -movflags +faststart \$out
...
if (${WANT_STILL} -eq 1) {
  ffmpeg -hide_banner -loglevel error -y -ss ([double]\$len * 0.4) -i \$out -frames:v 1 "\$dir\\frames\\final_still.png"
}
EOF
echo "render: $(since "$T0")"
if [ "${WANT_STILL}" = 1 ]; then
  mkdir -p "${MAC_STILLS}"
  pc_pull "$(ff "${DIR}")/frames/final_still.png" "${MAC_STILLS}/${NAME}_final.png"
