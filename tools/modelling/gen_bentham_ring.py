...
RING_LIFT = EYE_Y - RISE * (LEVEL_COUNT - 1)
...
W('[gd_scene load_steps=20 format=3]')
W('')
...
W('editor_description = "Visual decal only, as StartPad. It is on LEVEL THREE."')
W('size = Vector3(6.0, 0.1, 6.0)')
...
W('editor_description = %s' % repr_desc(
    "%.1f deg on level three's racing line, y=%.1f. This is the end of the whole route, not the "
    "end of a lap: reaching it is what takes the tower." % (exit_deg(), top_y)))
...
W('editor_description = %s' % repr_desc(
    ...
    "at that height it is in clear sky from all three galleries.\n\n"
