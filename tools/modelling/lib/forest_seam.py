...
great tree's leaf sheet springs on up off it like a dome off its wall, crests
just inside the ring (CREST) and curves down to the crown's rim: a dome
collapsed where the trunk holds it.
...
points are identical floats in both meshes: no seam, no gap, no step.
    ...
    sheet_z(rad)  the tower sheet's nominal underside at radius rad (before
                  its lumps): springs up off the drum's head, crests at CREST,
                  curves down to the crown rim; for the map's ray tracing
...
and the dome crests at 53.5 and sags from there to the crown rim at 34.1 -- a
19.4 m collapse into the tree. The guard's room, eye (28.7) and windows are untouched, and every
...
# Ryan: "it should be more concave like a dome, and it should meld better with the wall."
# The dome's crest ring (r, y): from the drum's head the leaf springs on up and in
